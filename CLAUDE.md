  ...
  "questions": [
    {
      ...
      "category": "出題範囲の説明", // カテゴリ名（ストラテジ系、マネジメント系、テクノロジ系など）
      "topic": "出題範囲",  // 「企業活動」「法務」「ソフトウェア」など
      "keyword": "ウォーターフォールモデル"  // 正解するために知っておくべき用語（短い用語・名称のみ）
    }
  ]
}
...
## アプリの機能
### 予想問題出題機能
情報処理技術者試験の各種試験の予想問題を出題するクイズアプリ。
各資格のカテゴリ（午前、午後、ストラテジ系など）ごとに10問ずつ出題する。
日付が変わったら新しい問題がClaude codeのスケジュールによって更新される

### デイリーミッション
10問正解（いずれかのカテゴリで全問正解する）すると、広告が非表示になる。
日付が変わったら再度表示されるので、広告を消すにはまた10問正解する。
